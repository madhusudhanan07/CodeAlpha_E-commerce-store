/**
 * recommendationController.js — Intelligent Recommendation Engine Logic (Firestore)
 *
...
// Helper to resolve user ID from token (returns Firebase UID)
...
export const getRelatedProducts = async (req, res, next) => {
  try {
    ...
    let related = all
      .filter((p) => p.category_slug === product.category_slug && String(p.id) !== String(productId));

    related = shuffleArray(related).slice(0, 6);

    // Fallback if category has fewer products
    if (related.length < 4) {
      const fallback = shuffleArray(
        all.filter((p) => String(p.id) !== String(productId)),
...
export const getRecommended = async (req, res, next) => {
  try {
    const all  = await ProductModel.findAll();
    const rows = shuffleArray(all).slice(0, 8);
